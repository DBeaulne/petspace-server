...
exports.seed = async function(knex) {
  ...
  await knex('sitter_bridge').insert([
    {
      id: 1,
      sitter_id: await knex('sitter').where('email', 'alice.johnson@example.com').select('account_id').first().then(row => row.account_id),
      pet_id: await knex('pet').where('name', 'Buddy').select('id').first().then(row => row.id)
    },
    {
      id: 2,
      sitter_id: await knex('sitter').where('email', 'bob.williams@example.com').select('account_id').first().then(row => row.account_id),
