...
exports.seed = async function(knex) {
  ...
  await knex('user_pet').insert([
    ...
    {
      id: 1,
      user_id: await knex('users').where('email', 'john.doe@example.com').select('id').first().then(row => row.id),
    ...
    {
      id: 3,
      user_id: await knex('users').where('email', 'jane.smith@example.com').select('id').first().then(row => row.id),
